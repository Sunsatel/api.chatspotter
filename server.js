...
const stripe = Stripe(import.meta.env.VITE_STRIPE_PUBLIC_KEY) //Stripe(import.meta.env.VITE_STRIPE_PUBLIC_KEY);
const app = express();
...
async function getTimezoneFromPostalCode(postalCode) {
  try {
    ...
    const geocodeResponse = await axios.get(
      `https://maps.googleapis.com/maps/api/geocode/json?address=${encodeURIComponent(postalCode)}&key=${GOOGLE_API}`
    );
    ...
    const timezoneResponse = await axios.get(
      `https://maps.googleapis.com/maps/api/timezone/json?location=${lat},${lng}&timestamp=${Math.floor(Date.now() / 1000)}&key=${GOOGLE_API}`
    );
 ...
 */
 app.post('/stripe/create-checkout-session', cors(), async (req, res) => {
...
app.get('/stripe/success.html', async (req, res) => {
  /*
...
*/
  ...
  async function createNewChatSpotterUser(){
     try {
        ...
        const timezone = await getTimezoneFromPostalCode(customer.address.postal_code) //array of time_zone,time_zone_offset_hr
    ...
    // send thank you page
    res.send(`<html><body><h1>Thanks for your order, ${customer.name}!</h1><p>Please check your email for a confirmation link.</p></body></html>`);
...
88       88Y888P' `88888P' dP         dP   `88888P8 dP
         ...
         dP
*/
app.post('/stripe/create-portal-session', cors(), async (req, res) => {
